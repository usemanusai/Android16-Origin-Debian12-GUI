...
| Android 16 + manual upgrade to 13 | Debian 13 (trixie) | ⚠️ | Works post-upgrade, but the bookworm→trixie dist-upgrade can corrupt the AVF VM image — see warnings in `update_debian13_EN.sh`. |
...
| `update_debian13.sh`    | Upgrades a Debian 12 system to Debian 13 (Chinese); exits cleanly if already on trixie        |
